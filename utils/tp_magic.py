...
class TPMagic:
    ...
    def __init__(self, tp1_pips: float, tp2_pips: float, tp3_pips: float, sl_pips: float,
                 ...
                 initial_lot: float = 0.01, strategy_name: str = "TPMagic", logger=None):
        ...
        # Load magic from config
        config_path = os.path.join(os.path.dirname(__file__), '../magic_numbers_reference.json')
        with open(config_path, 'r') as f:
            config = json.load(f)
        self.magic = config['magic_numbers'].get(self.strategy_name, 100000)
    ...
    def open_position(self, direction: int, entry_price: float, lot_size: Optional[float] = None) -> bool:
        ...
        if self.mode == TPMagicMode.LIVE:
            ...
            if symbol_info is None:
                self.logger.error(f"Symbol info not available for {self.symbol}")
                self._reset()
                return False
            if symbol_info.trade_mode == 0:  # SYMBOL_TRADE_MODE_DISABLED
                self.logger.error(f"Trading disabled for symbol {self.symbol}")
                self._reset()
                return False
            
            # Get fresh tick for current price
            tick = mt5.symbol_info_tick(self.symbol)
            if tick is None:
                self.logger.error("No tick data available")
                self._reset()
                return False
            
            current_price = tick.ask if direction == 1 else tick.bid
            self.logger.info(f"Using fresh market price: {current_price} (passed entry: {entry_price})")
            ...
            if symbol_info:
                if lot_size < symbol_info.volume_min:
                    ...
                    self.logger.warning(f"Lot size adjusted to min: {lot_size}")
                elif lot_size > symbol_info.volume_max:
                    lot_size = symbol_info.volume_max
                    self.logger.warning(f"Lot size adjusted to max: {lot_size}")
            ...
            self.current_sl = round(self.current_sl, digits)
            
            # Send initial order with valid initial SL, no TP (manage manually)
            ...
            request = {
                ...
                "type": action,
                "price": current_price,
                "sl": self.current_sl,  # Valid initial SL
                "tp": 0.0,
                "deviation": 20,
                "magic": self.magic,
                "comment": f"{self.strategy_name} Initial",
            ...
            if result is None:
                ...
                self.logger.error(f"MT5 order_send returned None: code {error_code}, message: {error_msg}")
                self._reset()
                return False
            elif result.retcode != mt5.TRADE_RETCODE_DONE:
                self.logger.error(f"Order send failed: retcode {result.retcode}, comment: {getattr(result, 'comment', 'N/A')}")
                self._reset()
                return False
            else:
                self.logger.info(f"Live position opened: {direction} {lot_size} lots at {current_price}, initial SL: {self.current_sl}")
                self.total_lot = lot_size  # Update with adjusted lot
                self.remaining_lot = lot_size
                return True
        else:
    ...
    def _execute_close(self, lot: float, price: float):
        ...
        our_pos = next((p for p in positions if p.magic == self.magic), None)
        if not our_pos:
        ...
        request = {
            ...
            "deviation": 20,
            "magic": self.magic,
            "comment": f"{self.strategy_name} Close {lot}",
            "type_time": mt5.ORDER_TIME_GTC,
            "type_filling": mt5.ORDER_FILLING_IOC,
        }
        result = mt5.order_send(request)
